...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $emailquery = "select * from product where email='$email' ";
        ...
        if ($emailcount > 0) {
            ...
            if ($password === $cpassword) {
                ...
                $insertquery = "insert into product( username, category, area, email, mobile, password, cpassword, status, token) VALUES('$username', '$category', '$area', '$email', '$mobile', '$pass', '$cpass', 'inactive', '$token')";
                ...
                if ($iquery) {
                    ...
                    email http://localhost/p/miniproject/serviceregformactivate.php?token=$token ";
...
<div class="form">
		...
		<form name="f1" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>" method="POST">
			<p>Name:</p>
			<input type="text" name="username" placeholder="Full Name" pattern="[A-Za-z ]{5,50}" title="Minimum length should be 5 and maximum is 20" required>
			<p>Place:</p><br>
			<select name="area">
				<option>---Place---</option required>
			...
			<br><p>E-Mail:</p>
			<input type="email" name="email" placeholder="Email Address" title="Email must contain '@' "  required><br>
			<p>Contact Number:</p>
